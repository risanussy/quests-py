...
# Endpoint untuk menambahkan item ke store
...
def add_store_item():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("INSERT INTO store (user_id, harga, nama, deskripsi, kategori) VALUES (%s, %s, %s, %s, %s)",
                   (user_id, harga, nama, deskripsi, kategori))
    mysql.connection.commit()
    cursor.close()
    
    return jsonify({'message': 'Store item added successfully'}), 201

# Endpoint untuk mendapatkan semua item di store
...
# Endpoint untuk mendapatkan item di store berdasarkan id
@app.route('/api/store/<int:item_id>', methods=['GET'])
def get_store_item_by_id(item_id):
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT * FROM store WHERE id = %s", (item_id,))
...
# Endpoint untuk memperbarui item di store berdasarkan id
...
def update_store_item(item_id):
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("UPDATE store SET user_id = %s, harga = %s, nama = %s, deskripsi = %s, kategori = %s WHERE id = %s",
                   (user_id, harga, nama, deskripsi, kategori, item_id))
    mysql.connection.commit()
    cursor.close()
    
    return jsonify({'message': 'Store item updated successfully'}), 200

# Endpoint untuk menghapus item di store berdasarkan id
@app.route('/api/store/<int:item_id>', methods=['DELETE'])
def delete_store_item(item_id):
    cursor = mysql.connection.cursor()
    cursor.execute("DELETE FROM store WHERE id = %s", (item_id,))
    mysql.connection.commit()
    cursor.close()
...
def add_transaction():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("INSERT INTO transaction (store_id, user_id, status) VALUES (%s, %s, %s)",
...
@app.route('/api/transaction/<int:trans_id>', methods=['GET'])
def get_transaction_by_id(trans_id):
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT * FROM transaction WHERE id = %s", (trans_id,))
...
@app.route('/api/transaction/<int:trans_id>', methods=['PUT'])
def update_transaction(trans_id):
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("UPDATE transaction SET store_id = %s, user_id = %s, status = %s WHERE id = %s",
                   (store_id, user_id, status, trans_id))
    mysql.connection.commit()
    cursor.close()
...
def delete_transaction(trans_id):
    cursor = mysql.connection.cursor()
    cursor.execute("DELETE FROM transaction WHERE id = %s", (trans_id,))
    mysql.connection.commit()
    cursor.close()
